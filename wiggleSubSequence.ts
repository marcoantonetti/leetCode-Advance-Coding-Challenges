...
function getWiggleSubsequence(nums: number[]): number {

    const [first, ...newArr] : number[] = nums

    if(newArr[0] == first ) newArr.shift()    

    let alternator: boolean = newArr[0] > first ? true : false
